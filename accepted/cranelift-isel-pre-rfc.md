...
verified given semantics of input and output languages. The more
restricted semantics can also be simpler to understand in some sense:
one is just providing equivalences.

(Side-note: There are additional questions in how backend verification
would work that require more research, and so it is somewhat hard to
give a definitive answer to what DSL features would be required for
...
layers rather than directly in the basic lowering rules. If that's the
case, then verification might best be done probabilistically with a
fuzzing-like approach, where we drive the compiler with inputs and the
compiler produces a "witness proof", or correspondence between input
and output. However, if we wish to *statically* verify *the lowering
rules themselves*, this task is easier when they are expression
equivalences.)
